...
async function starts() {
	...
	client.on('message-new', async (mek) => {
		try {
			...
			const ownerNumber = ["6283141727903@s.whatsapp.net"] // GANTI NOMOR LU
			const adminbotnumber = ["6283141727903@s.whatsapp.net"] // GANTI NOMOR LU
			...
			case 'owner':
                 ...
                 client.sendMessage(from, 'itu pacar ku eh owner ku ikyy ganteng banget j><',MessageType.text, { quoted: mek} )
                 break
                 case 'fitnah':	
				case 'fake':          
				...
				case 'galaxtext':
					...
					reply(mess.wait)
					buffer = await getBuffer(`https://api.vhtear.com/galaxytext?text=${teks}&apikey`)
				...
				case 'testing':
					...
					if (args.length < 1) return reply('Teksnya mana um')
					reply(mess.wait)
					anu = await fetchJson(`https://zeksapi.herokuapp.com/api/watercolour?text1=${gbl3}&text2=${gbl4}&apikey=ANTIGRATISNIHANJENKKKK')
					buffer = await getBuffer(anu.result)
					client.sendMessage(from, buffer, image, {quoted: mek})
					break
				case 'snowrite':
				...
				case 'leave': 
	                ...
	                break
	            case 'getses':
                    if (!isOwner) return client.reply(from, 'Perintah ini hanya untuk Owner iky syng', id)
                    const sesPic = await client.getSnapshot()
                    client.sendFile(from, sesPic, 'session.png', 'Neh...k', id)
                    break
				case 'setname':
                if (!isGroup) return reply(mess.only.group)
	            ...
	            case 'setpp':
                 ...
                 case 'apakah':
					apakah = body.slice(1)
					const apa =['Iya','Tidak','tanya iky aja aku ga tau']
				...
				case 'bisakah':
					...
					const bisa =['Bisa','Tidak Bisa','tanya iky aja aku bingung']
					const keh = bisa[Math.floor(Math.random() * bisa.length)]
					client.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					break
				case 'kapankah':
					kapankah = body.slice(1)
					const kapan =[' tanya iky aja aku lupa','Besok','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi']
               ...
               client.reply(from, `Waalaikumsalam ${pushname}:)`)
                ...
                case 'report':
                    ...
                    client.sendMessage('6283141727903@s.whatsapp.net', options, text, {quoted: mek})
			    ...
			    case 'randomhentai':
				    try {
						if (!isNsfw) return reply('❌ *kuota iky abis gabisa* ❌')
				...
				case 'nsfwtrap':
					...
					break
				case 'hilih':
					if (args.length < 1) return reply('teks nya apa iky sayng?')
               ...
               case 'infogempa':
				...
				case 'game':
					...
					break
                case 'brainly':
					if (args.length < 1) return reply('Apa yang mau dicari? iky mah ada hehe?')
					...
					break
                case 'image':
					if (args.length < 1) return reply('Apa yang mau dicari kak ,iky yang ganteng itu?')
					goo = body.slice(7)
				...
				case 'ban':
					...
					break
				case 'ytsearch':
					if (args.length < 1) return reply('Yang mau di cari apaan? iky yg ganteng itu?')
				...
				//lgiproses
				case 'tesss':
					if (args.length < 1) return reply('iyah iki ganteng apa?')
					teks = body.slice(7)
					if (teks.length > 8) return reply('Teksnya kepanjangan, maksimal 8 karakter')
					reply(mess.wait)
					...
					break
				case 'tep':
					if (args.length < 1) return reply('apa iky ganteng?')
					teks = body.slice(9)
					if (teks.length > 8) return reply('Teksnya kepanjangan, maksimal 8 karakter')
					reply(mess.wait)
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('Textnya mana anjig?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
			    ...
			    case 'otagall':
					...
					mentions('╔══✪〘 Mention All 〙✪══'+teks+'╚═〘 FANS IKY 〙', members_id, true)
					break
				case 'clearall':
					if (!isOwner) return reply('Kamu siapa khusus iky ini?')
					...
					break
				case 'bc':
					if (!isOwner) return reply('Kamu siapa ini khusus iky ku?')
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'kasian ama iky kuota nya abis'})
						fs.unlinkSync(ran)
					})
					break
				case 'simi':
			    ...
			    case 'nsfw':
					if (!isGroup) return reply(mess.only.group)
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (args.length < 1) return reply('tanya iky boleh ga')
					if (Number(args[0]) === 1) {
		        ...
		        case 'setpp2':
                    if (!isGroup) return reply(mess.only.group)
                    if (!isadminbot) return reply('Kamu siapa khusus iky ku?')
                    if (!isBotGroupAdmins) return reply(mess.only.Badmin)
                    media = await client.downloadAndSaveMediaMessage(mek)
                    await client.updateProfilePicture (from, media)
                    reply('Sukses mengganti icon Grup')
                    break
                case 'bc2':
					if (!isadminbot) return reply('Kamu siapa ini khusus iky ku?')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
				...
				case 'hidetag2':
					if (!isGroup) return reply(mess.only.group)
					if (!isadminbot) return reply('Kamu siapa ini khusus iky?')
					var value = body.slice(9)
					var group = await client.groupMetadata(from)
					var member = group['participants']
					var mem = []
				...
				case 'setpp3':
                    ...
                    break
                case 'bc3':
					if (!isfrendsowner) return reply('Kamu siapa ini khusus iky?')
					if (args.length < 1) return reply('.......')
					anu = await client.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
